...
$(OUTPUT_DIR)/%_schematic.svg: $(OUTPUT_DIR)/%_flattened_verilog.v
	python3 $(SCRIPT_DIR)/generate_schematic.py -input_file $< -output $(basename $@)
	mv parser.out $(OUTPUT_DIR)
	mv parsetab.py $(OUTPUT_DIR)

$(OUTPUT_DIR)/%_ast.log: $(OUTPUT_DIR)/%_flattened_verilog.v
	python3 $(SCRIPT_DIR)/ast_understanding.py -input_file $< | tee $@
	mv parser.out $(OUTPUT_DIR)
	mv parsetab.py $(OUTPUT_DIR)

$(OUTPUT_DIR)/%_flattened_verilog.v: $(DESIGN_DIR)/%.v
